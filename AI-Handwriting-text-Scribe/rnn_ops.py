...
# Compatibility shims for TensorFlow 2.x
def _like_rnncell(cell):
    """Check if cell has state_size and output_size properties."""
    return hasattr(cell, 'state_size') and hasattr(cell, 'output_size')

def _concat(prefix, suffix, static=False):
    """Concat that behaves like nest.flatten."""
    # Convert to lists
    if isinstance(prefix, (list, tuple)):
        p = list(prefix)
    elif isinstance(prefix, tensor_shape.TensorShape):
        p = list(prefix.as_list()) if prefix.dims is not None else []
    else:
        p = [prefix]
    ...
    if isinstance(suffix, (list, tuple)):
        ...
        s = list(suffix.as_list()) if suffix.dims is not None else []
    else:
        s = [suffix]
...
def _maybe_tensor_shape_from_tensor(shape):
    """Returns a TensorShape from a Tensor or TensorShape."""
